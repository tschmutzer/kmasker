...
public:
  ...
  bool                           threads_given;
  ::std::vector<const char *>    sam_arg;
  typedef ::std::vector<const char *>::iterator sam_arg_it;
  typedef ::std::vector<const char *>::const_iterator sam_arg_const_it;
  bool                           sam_given;
  uint32_t                       Files_arg;
  bool                           Files_given;
  ...
  int32_t                        quality_start_arg;
  bool                           quality_start_given;
  int32_t                        min_quality_arg;
  ...
  count_main_cmdline() :
    ...
    quality_start_arg((int32_t)64), quality_start_given(false),
...
#define CHECK_ERR(type,val,which) if(!err.empty()) { ::std::cerr << "Invalid " #type " '" << val << "' for [" which "]: " << err << "\n"; exit(1); }
    while(true) {
      ...
      case QUALITY_START_OPT:
        ...
        quality_start_arg = conv_int<int32_t>((const char*)optarg, err, false);
        CHECK_ERR(int32_t, optarg, "    --quality-start=int32")
        break;
      case MIN_QUALITY_OPT:
        min_quality_given = true;
        min_quality_arg = conv_int<int32_t>((const char*)optarg, err, false);
        CHECK_ERR(int32_t, optarg, "    --min-quality=int32")
        break;
      case 'p':
        reprobes_given = true;
    ...
    if(quality_start_given && min_qual_char_given)
      error("Switches [    --quality-start=int32] and [-Q, --min-qual-char=string] are mutually exclusive");
    if(min_quality_given && min_qual_char_given)
      error("Switches [    --min-quality=int32] and [-Q, --min-qual-char=string] are mutually exclusive");
...
#define PACKAGE_VERSION "0.0.0"
  ...
  void dump(::std::ostream &os = std::cout) {
    ...
    os << "sam_given:" << sam_given << " sam_arg:" << vec_str(sam_arg) << "\n";
